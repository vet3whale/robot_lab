...
def move_toward_goal_exp(
    ...
    """Dense "move toward the goal" reward - the position-task analog of track_lin_vel_xy_exp.

    The pose goal is stored in the base frame, so its xy gives the goal direction in that frame. We
    build a desired base-frame velocity pointing at the goal at ``max_speed`` (ramping down to 0 within
    ``max_speed`` metres of the goal so the robot can settle), then reward how well the actual base
    velocity matches it with the same exp kernel and upright gate the velocity task uses.

    Unlike position_command_error_tanh this stays graded everywhere - it does not saturate flat far
    from the goal - so it provides the always-on forward drive that keeps the action std bounded once
    the fall penalty and early terminations are removed to mirror the velocity task.
    """
    asset: RigidObject = env.scene[asset_cfg.name]
    goal_vec_b = env.command_manager.get_command(command_name)[:, :2]
    goal_dist = torch.norm(goal_vec_b, dim=1)
    goal_dir = goal_vec_b / (goal_dist.unsqueeze(1) + 1e-6)
    desired_vel_b = goal_dir * torch.clamp(goal_dist, max=max_speed).unsqueeze(1)
    vel_error = torch.sum(torch.square(desired_vel_b - asset.data.root_lin_vel_b[:, :2]), dim=1)
